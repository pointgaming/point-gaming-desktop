...
uint belowOrEquals(const uint G, const uint targetG)
{
	uchar* b;
	uchar* l;
...
__kernel void searchGeneral(	const uint block0, const uint block1, const uint block2,
						const uint state0, const uint state1, const uint state2, const uint state3,
						const uint state4, const uint state5, const uint state6, const uint state7,
						const uint target6, const uint base, __global uint * output)
{
	u nonce;
#ifdef VECTORS	
...
#ifdef VECTORS
	if((H.x==0) && (belowOrEquals(G.x, target6)))
	{
		output[0] = nonce.x;
	}
	if((H.y==0) && (belowOrEquals(G.y, target6)))
	{
		output[0] = nonce.y;
	}
#else

	if(H==0 && belowOrEquals(G, target6))
	{
		output[0] = nonce;
		output[1] = nonce;
	}
	
//	if (nonce==0) output[0] = H;
#endif
}
