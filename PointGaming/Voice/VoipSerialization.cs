...
namespace PointGaming.Voice
{
    ...
    {
        public static void WriteByte(byte[] buffer, ref int position, byte value)
        {
            buffer[position++] = value;
        }
        public static bool ReadByte(byte[] buffer, int length, ref int position, out byte value)
        {
            value = 0;
            if (position >= length)
                return false;
