...
namespace PointGaming.Voice
{
    class VoipMessageLeaveRoom : IVoipMessage
    {
        ...
        public bool Read(byte[] buffer, int position, int length)
        {
            ...
            VoipSession.VoipDebug(VoipSession.DebugPacketContent, string.Format("rx leave: roomname'{0}' issuccess '{1}'", RoomName, IsSuccess));
        ...
        public int Write(byte[] buffer, byte[] key)
        {
            ...
            VoipSession.VoipDebug(VoipSession.DebugPacketContent, "tx leave: uid[{0}] key[{1}] iv[{2}] plain[{3}] crypt[{4}]", suid, skey, siv, splain, scrypt);
